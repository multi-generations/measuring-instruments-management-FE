...
export class InstrumentDetailComponent implements OnInit {
    ...
    public setModalBodyDocumentVolatility(id: number) {
        ...
        modalBody += '<tr class="col-12">' +
            `<td class="col-1">${this.formatDate(deleteDocumentVolatility?.volatilityDate)}</td>` +
            `<td class="col-4">${deleteDocumentVolatility?.attachedDocument.documentName}</td>` +
    ...
    formatDate(curDate: Date | undefined): string | null {
        if (curDate) {
            return new DatePipe('en-US').transform(curDate, 'dd/MM/yyyy');
        }
        return null;
    }
}
